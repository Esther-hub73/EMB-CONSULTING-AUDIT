...
function Header({ onHome, onDashboard, showDashboardLink }) {
  return (
    <header className="print:hidden sticky top-0 z-20 border-b" style={{ backgroundColor: COLORS.ink, borderColor: COLORS.ink }}>
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between">
        ...
        {showDashboardLink && (
          <button onClick={onDashboard} className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border" style={{ borderColor: COLORS.brassLight, color: COLORS.brassLight }}>
            Tableau de bord
          </button>
        )}
      </div>
...
export default function App() {
  ...
  const [view, setView] = useState('home'); // home | dashboard | newEst | estDetail | audit | report | admin | alertes | formation | amelioration
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: COLORS.ivory, fontFamily: F_BODY }}>
      <Header onHome={() => setView('home')} onDashboard={() => setView('dashboard')} showDashboardLink={view !== 'home' && view !== 'dashboard'} />
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-6 pb-16">
        {view === 'home' && (
          <Home establishments={establishments} audits={audits} onGoDashboard={() => setView('dashboard')} onNew={() => setView('newEst')} />
        )}
        {view === 'dashboard' && (
          <Dashboard
...
function Home({ establishments, audits, onGoDashboard, onNew }) {
